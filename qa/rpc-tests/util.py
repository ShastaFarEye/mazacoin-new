...
def initialize_datadir(dir, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "mazacoin.conf"), 'w') as f:
        f.write("regtest=1\n");
        f.write("rpcuser=rt\n");
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    mazacoind and mazacoin-cli must be in search path.
    """
    ...
    if not os.path.isdir(os.path.join("cache", "node0")):
        ...
        for i in range(4):
            ...
            args = [ "mazacoind", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            bitcoind_processes[i] = subprocess.Popen(args)
            subprocess.check_call([ "mazacoin-cli", "-datadir="+datadir,
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in mazacoin.conf
...
def start_node(i, dir, extra_args=None, rpchost=None):
    """
    Start a mazacoind and return RPC connection to it
    """
    datadir = os.path.join(dir, "node"+str(i))
    args = [ "mazacoind", "-datadir="+datadir, "-keypool=1", "-discover=0" ]
...
def start_nodes(num_nodes, dir, extra_args=None, rpchost=None):
    """
    Start multiple mazacoinds, return RPC connections to them
    """
